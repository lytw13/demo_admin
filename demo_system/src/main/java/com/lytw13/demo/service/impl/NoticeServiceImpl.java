...
import com.lytw13.demo.mapper.sys.NoticeMapper;
...
public class NoticeServiceImpl implements NoticeService {
    ...
    public BaseResult get(@PathVariable("id") Integer id) {
        ...
        if(notice==null) {
            return new ResponseResult().setResultFail("未查到公告信息");
        }
        return new ResponseResult().setResultSuccess(notice);
    }

    @Override
    public BaseResult getNotice(TbNotice tbNotice) {
        List<TbNotice> noticeList = noticeMapper.getNotice(tbNotice);
        if(noticeList.size() == 0) {
            return new ResponseResult().setResultFail("未查到公告信息");
        }
        return new ResponseResult().setResultSuccess(noticeList);
    }
